...
TEST_F(MileageFileHandlerTest,
  ...
  EXPECT_CALL(mockFileController,
              open(_, QIODevice::WriteOnly | QIODevice::Text))
      .WillOnce(Return(true));

  // Expect writing "0.0" to file
  EXPECT_CALL(mockFileController, write(_, QString("0.0")))
      .WillOnce(Return(true));
...
TEST_F(MileageFileHandlerTest, ReadMileage_ValidNumber_ReturnsParsedValue) {
  EXPECT_CALL(mockFileController,
              open(testing::_, QIODevice::ReadOnly | QIODevice::Text))
...
TEST_F(MileageFileHandlerTest, WriteMileage_ValidNumber_WritesToFile) {
  EXPECT_CALL(mockFileController,
              open(testing::_, QIODevice::WriteOnly | QIODevice::Text))
      .WillOnce(Return(true)); // Simulate file opening
