...
TEST_F(ClusterSettingsManagerTest, ToggleDrivingModeEmitsSignal) {
  QSignalSpy drivingModeSpy(clusterSettingsManager,
  ...
  ASSERT_EQ(drivingModeSpy.takeFirst().at(0).value<DrivingMode>(),
...
TEST_F(ClusterSettingsManagerTest, ToggleClusterThemeEmitsSignal) {
  QSignalSpy themeSpy(clusterSettingsManager,
  ...
  ASSERT_EQ(themeSpy.takeFirst().at(0).value<ClusterTheme>(),
            ClusterTheme::Light);
...
TEST_F(ClusterSettingsManagerTest, ToggleClusterMetricsEmitsSignal) {
  QSignalSpy metricsSpy(clusterSettingsManager,
  ...
  ASSERT_EQ(metricsSpy.takeFirst().at(0).value<ClusterMetrics>(),
            ClusterMetrics::Miles);

  // Toggling again should switch back to Kilometers
  clusterSettingsManager->toggleClusterMetrics();
  ASSERT_EQ(metricsSpy.count(), 1);
  ASSERT_EQ(metricsSpy.takeFirst().at(0).value<ClusterMetrics>(),
            ClusterMetrics::Kilometers);
}
