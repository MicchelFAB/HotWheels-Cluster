...
protected:
  ...
  void SetUp() override {
    systemManager =
        new SystemManager(&mockBatteryController, &mockInfoProvider, nullptr);
  }
...
TEST_F(SystemManagerTest, UpdateSystemStatus_EmitsWifiStatus) {
  ...
  EXPECT_CALL(mockInfoProvider, getWifiStatus(_))
      .WillOnce(DoAll(
          testing::SetArgReferee<0>(mockWifiName), // Set wifiName argument
...
TEST_F(SystemManagerTest, UpdateSystemStatus_EmitsTemperature) {
  EXPECT_CALL(mockInfoProvider, getTemperature())
      .WillOnce(Return(QString("42.0°C")));
...
TEST_F(SystemManagerTest, UpdateSystemStatus_EmitsBatteryPercentage) {
  EXPECT_CALL(mockBatteryController, getBatteryPercentage())
...
TEST_F(SystemManagerTest, UpdateSystemStatus_EmitsIpAddress) {
  EXPECT_CALL(mockInfoProvider, getIpAddress())
