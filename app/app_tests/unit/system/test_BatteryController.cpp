...
TEST_F(BatteryControllerTest, GetBatteryPercentage_CorrectCalculation) {
  EXPECT_CALL(mockI2C, readRegister(0x02))
      .WillOnce(Return(1000)); // Bus voltage raw value
  EXPECT_CALL(mockI2C, readRegister(0x01))
      .WillOnce(Return(100)); // Shunt voltage raw value
