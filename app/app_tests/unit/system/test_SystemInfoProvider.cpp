...
protected:
  ...
  void SetUp() override {
    infoProvider = new SystemInfoProvider(&mockExecutor);
  }
...
TEST_F(SystemInfoProviderTest, GetWifiStatus_Connected) {
  QString wifiName;
  EXPECT_CALL(mockExecutor, executeCommand(QString(
...
TEST_F(SystemInfoProviderTest, GetTemperature_ValidReading) {
  EXPECT_CALL(mockExecutor,
              readFile(QString("/sys/class/hwmon/hwmon0/temp1_input")))
...
TEST_F(SystemInfoProviderTest, GetIpAddress_Valid) {
  EXPECT_CALL(mockExecutor, executeCommand(QString(
                                "sh -c \"ip -4 addr show wlan0 | grep -oP "
                                "'(?<=inet\\s)\\d+\\.\\d+\\.\\d+\\.\\d+'\"")))
