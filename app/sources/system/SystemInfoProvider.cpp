...
QString SystemInfoProvider::getWifiStatus(QString &wifiName) const {
  QString output =
      m_executor->executeCommand("nmcli -t -f DEVICE,STATE,CONNECTION dev");
...
QString SystemInfoProvider::getTemperature() const {
  QString tempStr =
      m_executor->readFile("/sys/class/hwmon/hwmon0/temp1_input").trimmed();
...
QString SystemInfoProvider::getIpAddress() const {
  QString output =
      m_executor->executeCommand("sh -c \"ip -4 addr show wlan0 | grep -oP "
                                 "'(?<=inet\\s)\\d+\\.\\d+\\.\\d+\\.\\d+'\"");
