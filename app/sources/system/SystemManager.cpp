...
SystemManager::SystemManager(IBatteryController *batteryController,
    ...
    : QObject(parent),
      ...
      m_systemInfoProvider(systemInfoProvider ? systemInfoProvider
                                              : new SystemInfoProvider()),
      m_systemCommandExecutor(systemCommandExecutor
...
void SystemManager::updateSystemStatus() {
  QString wifiName;
  emit wifiStatusUpdated(m_systemInfoProvider->getWifiStatus(wifiName),
                         wifiName);
